...
// Create a single function call from a list of normalizer names.
// Normalizers are applied left-to-right as specified: [a, b, c] => c(b(a(value)))
function composeNormalizer(normalizers) {
  return normalizers.reduceRight(function(previousValue, currentValue) {
    if(!supportedNormalizers[currentValue]) {
      throw 'No normalizer called ' + currentValue + ' found';
    }
    return function(value) { return previousValue(supportedNormalizers[currentValue](value)); };
...
function evalTest( testCase, apiResponse, context ){
  ...
  if ( !_.isObject(apiResponse) || apiResponse.length === 0 ) {
    return {
      result: 'fail',
