 ...
 */
function execTestSuites( config, callback ){
  var suiteResults = {
    stats: {
      pass: 0,
      fail: 0,
  ...
  var testSuites = filterTestsByType(config.testSuites);
  globalStats.testsTotal = countTotalTests(testSuites);
  var numTestSuites = testSuites.length;

  testSuites.forEach(function(suite) {
    execTestSuite( config.endpoint.url, suite, globalStats, function ( testResults ){
