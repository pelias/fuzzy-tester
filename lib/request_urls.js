...
function printProgress(done, total) {
  const status_code_output = Object.keys(status_codes).map(function(code){
    return `${code}: ${status_codes[code]}`;
  }).join(',');
  process.stderr.write(
    `\rTests completed: ${done}/${total} (retries: ${retries}${retries > 0 ? ', '+ status_code_output :''})`
  );
  if (done === total) {
  ...
  var getOneUrl = function (){
    ...
    request( requestOpts, function ( err, res ){
      if( err ){
        ...
        urls.push(url);
        retries++;
        if (status_codes[res.statusCode] === undefined) {
          status_codes[res.statusCode] = 1;
        } else {
