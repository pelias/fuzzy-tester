...
function scoreOneResult(expected, apiResult, apiResults, context) {
  ...
  // set priorityThresh to 0 when properties score is also 0
  // otherwise, tests that completely fail will still get points for the priorityThresh
  if (subscores[0].score === 0) {
    subscores[1].score = 0;
  }
